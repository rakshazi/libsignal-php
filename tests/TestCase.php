        ...
        switch (\gettype($obj)) {
            case 'object':
                ...
                foreach ($vars as $key => $var) {
                    $type = '';
                    $k = \bin2hex($key);
                    if (0 === \strpos($k, '002a00')) {
                        $k = \str_replace('002a00', '', $k);
                        $type = ':protected';
                    } elseif (0 === \strpos($k, \bin2hex("\x00".\get_class($obj)."\x00"))) {
                        $k = \str_replace(\bin2hex("\x00".\get_class($obj)."\x00"), '', $k);
                        $type = ':private';
                    }
                    $k = \hex2bin($k);
                    if (\is_subclass_of($obj, 'ProtobufMessage') && 'values' === $k) {
                        $r = new ReflectionClass($obj);
                        $constants = $r->getConstants();
                        $newVar = [];
                        foreach ($constants as $ckey => $cval) {
                            if ('PB_' !== \substr($ckey, 0, 3)) {
